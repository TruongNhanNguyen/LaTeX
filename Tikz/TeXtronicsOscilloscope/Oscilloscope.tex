...
\usepackage[marfin=1in]{geometry}
\usepackage{tikz}
...
\begin{document}
   \begin{tikzpicture}[
                ...
                axes/.style={thick}]
            \fill[green!30!blue!30!,rounded corners,draw=black,thick](0,0) rectangle (27.75,13.25); % Vẽ khung ngoàingoài cùng của máy 
            \fill[fill=black!40!,draw=black,thick,rounded corners](0.25,0.25) rectangle (27.5,13.00);% Vẽ khung trong của máy 
         \begin{scope}[xshift=7cm,yshift=8cm,samples=150] % Dịch chuyển vùng vẽ lên điểm (7,8)
            \fill[black!60!,rounded corners,draw=black,thick](-5.3,-4.3)rectangle (5.3,4.3);  % Vẽ khung ngoài màn hình hiển thịthị
            \fill[screen] (-5.0,-4.0) rectangle (5.0,4.0);% Vẽ khung trong màn hình hiển thị
            \draw[thin] (-5.0,-4.0) grid (5.0,4.0);
            \draw[axes] (-5,0)--(5,0); % Vẽ hệ trục thời gian và chia tỉ lệ cho trụctrục
            \draw[axes] (0,-4)--(0,4); %
            \foreach \i in {-4.8,-4.6,...,4.8} \draw (\i,-0.1)--(\i,0.1);
            \foreach \i in {-3.8,-3.6,...,3.8} \draw (-0.1,\i)--(0.1,\i);
            \draw[trace] plot(\x,{1+2.4*sin((2.5*\x +1) r)}); % Vẽ đồ thị hình sin r là đơn vị radrad
            \draw[trace] plot(\x,{-1+1.25*sin((0.75*\x) r});
         
            
        \end{scope}
        % Vẽ chân đế cho máy đo
    \fill[black!70!,rounded corners,xshift=2cm] (0,-.5) rectangle (2,0);
    \fill[black!70!,rounded corners,xshift=23.75cm] (0,-.5) rectangle (2,0);     
        % Bên trái dưới bản điều khiển 
    \fill[controlpanels] (0.6,0.5) rectangle (13.5,3.0);
    \path (0.8,0.9) node[scale=\scl,right]{$\mathbf{TeXtronics\,1 - v.1.01}$};
        % Góc dưới bên phải máy đo (Bảng điều khiển 1)
